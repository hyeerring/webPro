...
insert into emp values (1111, 'HONG', 'manager', 900, null);
select * from emp;
...
create table student(
    ...
    mCode		int,
    primary key(sNo),
    foreign key(mCode) references major(mCode)); -- 외래키 제약 조건은 아래에 작성
    
select * from student;
select * from major;

insert into major (mName, mOffice) values ('컴퓨터공학', '202호'); -- mCode 1
insert into major (mName, mOffice) values ('빅데이터', '302호'); -- mCode 2
insert into major (mName, mOffice) values ('디자인', '402호'); -- mCode 3
select * from major;

insert into student values (1000, '김길동', 1);
insert into student values (1001, '일길동', 2);
insert into student values (1002, '이길동', 3);
insert into student values (1003, '삼길동', 4); -- error
select * from student;
...
insert into division values (20, 'reserch', '02-555-4321', '강남');
...
-- 7. 사번, 이름, 상사이름
select w.pno, w.pname name, m.pname manager
...
select w.pno, w.pname name, m.pname manager
    ...
    on w.manager = m.pno;
-- 8-1. 사번, 이름, 상사이름, 부서명
select w.pno, w.pname name, m.pname manager, dname
...
select w.pno, w.pname, w.pay, dname, m.pname
	from personal w, personal m, division
    where w.manager = m.pno and w.dno = division.dno;
-- 11. 부서코드, 급여합계, 최대급여
select dno, sum(pay) sum_pay, max(pay) max_pay
	from personal
    group by dno;
-- 12. 부서명, 급여평균, 인원수
select dname, avg(pay) avg_pay, count(*) count
	from personal, division
    where personal.dno = division.dno
    group by dname;
-- 13. 부서코드, 급여합계, 인원수 인원수가 4명 이상인 부서만 출력
select dno, sum(pay) sum_pay, count(*) count
	from personal
    group by dno
    having count(*) >= 4;
...
select pno, pname, pay, dname
    ...
    order by personal.dno, pay desc;
-- 17. 자신이 속한 부서의 평균보다 많인 받는 사람의 이름, 급여, 부서번호, 반올림한 해당부서평균
select p.pname, p.pay, p.pno, round(avgpay)
...
-- 19. 이름, 급여, 해당부서평균
select p.pname, p.pay, avgpay
	from personal p, (select dno, avg(pay) avgpay from personal group by dno) a
    where p.dno = a.dno;
-- 20. 이름, 급여, 부서명, 해당부서평균
select p.pname, p.pay, dname, avgpay
	from personal p, division, 
		(select dno, avg(pay) avgpay from personal group by dno) a
    where p.dno = a.dno and p.dno = division.dno;

-- Oracle에서의 단일행 함수와 다른 부분
select curdate();
insert into personal values (1000, 'hong', 'manager', 1001, curdate(), null, null, 40);
select * from personal;

-- 데이터 삭제
set sql_safe_updates = 0;
delete from personal where pname = 'hong'; -- error: Workbench에서 데이터의 삭제를 막음

	-- ex. "____은 job이다." 출력
select concat(pname, '는 ', job, '이다.') msg from personal;
select round(35.678); -- from절 없이도 실행 가능
...
select dayname(now());
select pname, monthname(startdate) from personal; -- 월 영문표기
...
-- date_format(날짜/시간, 포맷) --> 문자
	...
	-- 포맷
    -- %Y : 연도 4자리		%y : 연도 2자리
    -- &m : 01, 02, 03		%c : 1, 2, 3		%M : 월 이름(January)			%b : 짧은 월 이름(JAN)		
    -- %d : 01, 02, 03		%e : 1, 2, 3
