...
<body>
	...
	try {
		...
		while (paramNames.hasMoreElements()) {
			...
			System.out.println("파라미터 이름: " + param + " / 서버에 저장된 파일 이름: " + filename + " / 오리지널 파일 이름: " + originalFilename);
		}
	} catch (IOException e) {
		System.out.println(e.getMessage());
	}

	// 서버에 업로드 후 소스폴더로 file copy
	InputStream is = null;
	...
	try {
		...
		if (serverFile.exists()) {
			...
			os = new FileOutputStream("D:/programming/06_jsp/ch14_fileUp/WebContent/fileUpFolder/" + filename); // 소스폴더로 복사될 파일
	...
	<h3>
		서버에 저장된 파일 이름:
		<%=filename%></h3>
	<img src="<%=conPath%>/fileUpFolder/<%=filename%>" alt="책 사진">
	<%
		} else {
	out.println("<h2>첨부 안함 filename: " + filename + "</h2>");
	}
	%>
</body>
</html>