...
public class Ex3_LifeCycle extends HttpServlet {
	...
	private void postConstruct() {
		// 서블릿 객체가 생성되자마자 실행
		System.out.print("Ex3_LifeCycle 서블릿 객체가 생성될 때 수행 -1");
	}
	public void init(ServletConfig config) throws ServletException {
		// 서블릿 객체가 생성될 때 실행
		System.out.print("Ex3_LifeCycle 서블릿 객체가 생성(메모리에 load)될 때 수행 -2");
	}
	public void destroy() {
		// 서블릿 객체가 소멸될 때 실행
		System.out.print("Ex3_LifeCycle 서블릿 객체가 소멸(메모리에서 사라질 때)될 때 수행 -1");
	}
	@PreDestroy
	private void PreDestroy () {
		// 서블릿 객체가 소멸되기 직전 실행
		System.out.print("Ex3_LifeCycle 서블릿 객체가 소멸되기 바로 전 수행 -2");
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		response.setContentType("text/html; charset=UTF-8");
		response.getWriter().append("<h1>doGet() 수행</h1>W");
